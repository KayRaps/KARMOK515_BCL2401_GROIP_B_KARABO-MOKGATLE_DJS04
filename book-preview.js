...
class BookPreview extends HTMLElement {
  ...
  addEventListeners() {
    ...
    this.listItems.addEventListener("click", this.handleListItemClick.bind(this));
    this.listClose.addEventListener("click", () => this.closeOverlay(this.listActive));
    this.searchCancel.addEventListener("click", () => this.closeOverlay(this.searchOverlay));
    this.settingsCancel.addEventListener("click", () => this.closeOverlay(this.settingsOverlay));
    this.searchForm.addEventListener("submit", this.handleSubmitSearch.bind(this));
    this.settingsForm.addEventListener("submit", this.handleSubmitSettings.bind(this));
  }

  initializeData() {
    this.createOptions(genres, "All Genres", this.searchGenres);
    this.createOptions(authors, "All Authors", this.searchAuthors);
    this.applyTheme(window.matchMedia("(prefers-color-scheme: dark)").matches ? "night" : "day");
  ...
  applyTheme(theme) {
    ...
    this.shadowRoot.host.style.setProperty("--color-dark", isNight ? "255, 255, 255" : "10, 10, 20");
    this.shadowRoot.host.style.setProperty("--color-light", isNight ? "10, 10, 20" : "255, 255, 255");
  }
  ...
  handleShowMore() {
    this.createBookPreviews(
      this.matches.slice(this.page * BOOKS_PER_PAGE, (this.page + 1) * BOOKS_PER_PAGE)
    );
    this.page += 1;
    this.updateShowMoreButton();
  }
  ...
  handleListItemClick(event) {
    ...
    if (active) {
      ...
      if (book) {
        ...
        this.listSubtitle.innerText = `${authors[book.author]} (${new Date(book.published).getFullYear()})`;
